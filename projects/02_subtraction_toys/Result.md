...
void app_main(void)
{
    ...
    int toys_at_home = 15;        // ของเล่นที่น้องมีอยู่
    ...
    // โจทย์เพิ่มเติม: แจกของเล่นให้เพื่อน 10 คน คนละ 2 ชิ้น
    int friends = 10;
    int toys_per_friend = 2;
    int toys_needed = friends * toys_per_friend;
    int current_toys = 15;  // สมมติว่าน้องมีของเล่น 15 ชิ้น
    int shortage = toys_needed - current_toys;

    ESP_LOGI(TAG, "🧑‍🤝‍🧑 โจทย์พิเศษ:");
    ESP_LOGI(TAG, "   น้องมีของเล่น %d ชิ้น", current_toys);
    ESP_LOGI(TAG, "   ต้องการแจกให้เพื่อน %d คน คนละ %d ชิ้น", friends, toys_per_friend);
    ESP_LOGI(TAG, "   ต้องใช้ของเล่นทั้งหมด: %d ชิ้น", toys_needed);

    if (current_toys >= toys_needed) {
        ESP_LOGI(TAG, "✅ ของเล่นเพียงพอสำหรับแจกทุกคน");
    } else {
        ESP_LOGW(TAG, "❌ ของเล่นไม่พอ! ขาดอีก %d ชิ้น", shortage);
    }
    ESP_LOGI(TAG, "");
    ...
    ESP_LOGI(TAG, "   ของเล่นเดิม: 🧸🚗🎲🧩🎮🧸🚁🎯 (%d ชิ้น)", toys_at_home);
